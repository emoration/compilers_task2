...
# 刷新模块, 防止修改语法文件后augmented_grammar没有更新
importlib.reload(my_grammar)
...
# %%
# 检查文法变量的格式
def check_grammar(_augmented_grammar: dict[str:list[list[str]]]):
    """
...
def get_augmented_grammar_to_index(_augmented_grammar: dict[str:list[list[str]]]) -> \
        tuple[dict[tuple[str, tuple[str]]:int], list]:
    """
    将文法按顺序编号，便于归约时输出r1r2r3....
    :param _augmented_grammar: 增广文法
    ...
    _augmented_grammar_order_list = list(map(
        lambda production_index: production_index[0],
        sorted(list(_augmented_grammar_to_index.items()), key=lambda x: x[1])
    ))
        ...
        for right in _augmented_grammar[A]:
            ...
            for i in range(len(right) - 1):
                ...
                if B in _nonterminal_symbol_list:
                    _follow[B] = _follow[B] | _first[beta[0]] - {'ε'}
        ...
        for A in _nonterminal_symbol_list:
            for right in _augmented_grammar[A]:
                ...
                for i in range(len(right)):
                    B = right[i]
                    if B in _nonterminal_symbol_list:
                        if i == len(right) - 1:  # B位于产生式最后，即A->αB
                            if _follow[A] - _follow[B] != set():
                                _follow[B] = _follow[B] | _follow[A]
                                is_changed = True
                        else:
                            beta = right[i + 1:]
                            if beta[0] in _nonterminal_symbol_list:  # B后面是非终结符，即A->αBβ
                                if 'ε' in _first[beta[0]]:  # FIRST(β)包含ε
                                    if _follow[A] - _follow[B] != set():
                                        _follow[B] = _follow[B] | _follow[A]
                                        is_changed = True
    return _follow
    ...
    for itemSet in _itemSetFamily:
        ...
        for X in _symbol_list:
            ...
            if len(goto_itemSet) > 0:
                ...
                # 如果X是终结符，goto加一条'跳转'边
                if X in _augmented_grammar.keys():
                    _goto[itemSet_index][X] += str(goto_itemSet_index)
                # 如果X是非终结符，action加一条'移入'边
                else:
                    _action[itemSet_index][X] += 's' + str(goto_itemSet_index)
        # 试图归约 # FIXME 将归约全部叠加到表格中，不处理冲突，冲突在后面的函数中处理
            ...
            if reduce_item.left_symbol == "S'":  # 如果是S'->S，那么acc
                ...
                for X in _terminal_symbol_list:
                    if X in _follow[reduce_item.left_symbol]:  # 如果X在FOLLOW(A)中，才会归约
                        _action[itemSet_index][X] += 'r' + str(reduce_item_index)
...
def SLR_parsing(_tokens: list[Token], _token_place: list[tuple[int, int]],
                _augmented_grammar_order_list: list[tuple[str, list[str]]],
    ...
    while True:
        ...
        if _next_action == '':
            ...
            if j >= len(_input) - 1:
                ...
                j -= 1  # 因为结尾有一个'$'
            # 3. 指针i向前移直到set(["block", "(closed_/open_)statement", "function", "program"] & goto[stack[i]] != set()。如果指针i前移过程中，在symbols[i]中遇到了这些非终结符中的某一个，那么匹配的非终结符优先级更低(防止死循环)
            target_goto_key_list = ["Block", "Closed_statement", "Open_statement", "Statement", "Function", "Program"]
            goto_key = ""
            while i >= 0:
...
if __name__ == '__main__':
    for file_id in ["1", "2"]:
        ...
        # 读取文法
        from my_grammar import augmented_grammar
